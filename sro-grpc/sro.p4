...
control MyIngress(inout headers hdr,
    ...
    apply {
        if (hdr.packet_out.isValid()){
            //todo: if needed, check for opcode 
        ...
        if(hdr.cpu.isValid()){
            switch (hdr.cpu.op){
                ...
                controllerOpcode_t.RESET_SEQ:{
                    ...
                    log_msg("setting seq to {}",{hdr.cpu.operand0});
                }
            }           
        }
            ...
            // packets from cpu are packet_out  
            else if(standard_metadata.ingress_port == CPU_PORT){
                //log_msg("debug: ipv4 packet from cpu port");
                drop();
                bit<8> id;
                switch_id.read(id,0);
                switch(id){
